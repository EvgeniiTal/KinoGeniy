    ...
    <>
      ...
      <div className="d-flex mt-5 gap-5">
        ...
        <div className="w-50 h-50 d-flex flex-column justify-content-between">
          <p>Жанр: {movie?.genres.map((genre: object) => genre.name).join(', ')}</p>
          <p>Год: {movie?.year}</p>
          <p>Страна: {movie?.countries.map((country: object) => country.name).join(', ')}</p>
        ...
        <div className="w-50 h-50 d-flex flex-column">
          <h3>Актеры:</h3>
          {movie?.persons.map((person: object) => (
