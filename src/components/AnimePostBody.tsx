...
import { fetchAnime } from '../redux/AnimeList-slice'
import { useDispatch, useSelector } from 'react-redux'
import { useEffect } from 'react'

export function AnimePostBody () {
    ...
    <>
      ...
      <div className="d-flex mt-5 gap-5">
        ...
        <div className="w-50 h-50 d-flex flex-column justify-content-between">
          <p>Жанр: {anime?.genres.map((genre: object) => genre.name).join(', ')}</p>
          <p>Год: {anime?.year}</p>
          <p>Страна: {anime?.countries.map((country: object) => country.name).join(', ')}</p>
        ...
        <div className="w-50 h-50 d-flex flex-column">
          <h3>Актеры:</h3>
          {anime?.persons.map((person: object) => (
