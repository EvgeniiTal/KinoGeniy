...
export function CartoonPostBody (): JSX.Element {
    ...
    <>
      ...
      <div className="d-flex mt-5 gap-5">
        ...
        <div className="w-50 h-50 d-flex flex-column justify-content-between">
          <p>Жанр: {cartoon?.genres.map((genre: object | any) => genre?.name).join(', ')}</p>
          <p>Год: {cartoon?.year}</p>
          <p>Страна: {cartoon?.countries.map((country: object | any) => country?.name).join(', ')}</p>
        ...
        <div className="w-50 h-50 d-flex flex-column">
          <h3>Актеры:</h3>
          {cartoon?.persons.map((person: object | any) => (
